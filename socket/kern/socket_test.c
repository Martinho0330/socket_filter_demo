...
int sock_filter(struct __sk_buff *skb)
{
  ...
  if (lookup_leaf)
  {
    ...
    __sync_lock_test_and_set(&lookup_leaf->timestamp, zero.timestamp);
    if (zero.is_request == 0) {
      __sync_fetch_and_add(&lookup_leaf->payload_length, payload_length);
    } else {
      __sync_fetch_and_add(&lookup_leaf->is_request, zero.is_request);
    }
    if (lookup_leaf->buf[0] == '0' && zero.buf[0] != '0') {
      __sync_lock_test_and_set(&lookup_leaf->buf[0], zero.buf[0]);
      __sync_lock_test_and_set(&lookup_leaf->buf[1], zero.buf[1]);
      __sync_lock_test_and_set(&lookup_leaf->buf[2], zero.buf[2]);
      __sync_lock_release(&lookup_leaf->buf[0]);
      __sync_lock_release(&lookup_leaf->buf[1]);
      __sync_lock_release(&lookup_leaf->buf[2]);
    }
    keySub.init_ts = lookup_leaf->init_ts;
    __sync_lock_release(&lookup_leaf->timestamp);
    bpf_map_update_elem(&state, &keySub, &zero, BPF_ANY);
    if (lookup_leaf->is_http == 1) {
      goto KEEP;
    }
    goto DROP_WITHOUT_INSERT;
  }
...
// keep the packet and send it to userspace returning -1
  ...
  {
    __sync_lock_test_and_set(&lookup_leaf->timestamp, zero.timestamp);
    __sync_lock_test_and_set(&lookup_leaf->is_http, zero.is_http);
    if (zero.is_request == 0) {
      __sync_fetch_and_add(&lookup_leaf->payload_length, payload_length);
    } else {
      __sync_fetch_and_add(&lookup_leaf->is_request, zero.is_request);
    ...
    __sync_lock_release(&lookup_leaf->timestamp);
    __sync_lock_release(&lookup_leaf->is_http);
    keySub.init_ts = lookup_leaf->init_ts;
    __sync_lock_release(&lookup_leaf->timestamp);
  } else {
...
// drop the packet returning 0
  ...
  if (lookup_leaf)
  {
    ...
    if (zero.is_request == 0) {
      ...
      __sync_lock_test_and_set(&lookup_leaf->is_request, zero.is_request);
      __sync_lock_release(&lookup_leaf->is_request);
    }
    if (lookup_leaf->buf[0] == '0' && zero.buf[0] != '0') {
      __sync_lock_test_and_set(&lookup_leaf->buf[0], zero.buf[0]);
      __sync_lock_test_and_set(&lookup_leaf->buf[1], zero.buf[1]);
